...
/// - starting an async D-Bus connection to the system bus and registering the service
/// - pipes calls of D-Bus methods to the background tasks so they are acted upon
///
/// # Errors
/// Will error out if the CEC or D-Bus connections could not be established.
#[instrument]
pub async fn run(
    device_name: String,
    ...
    // Setup bounded channel between D-Bus interface and background task.
    ...
    // that dropping the D-Bus connection would be bad?
    tracing::info!("Opening D-Bus connection to serve object {object_name}...");
    ...
    tracing::info!("D-Bus interface ready!");
    task_handle.await?;
